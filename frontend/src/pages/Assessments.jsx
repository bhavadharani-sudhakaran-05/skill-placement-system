...
const Assessments = () => {
  ...
  const styles = {
    ...
    categoriesRow: { display: 'flex', gap: '0.75rem', marginBottom: '2rem', flexWrap: 'wrap' },
    categoryChip: (active) => ({ padding: '0.5rem 1.25rem', borderRadius: '25px', border: 'none', cursor: 'pointer', fontWeight: 500, fontSize: '0.875rem', transition: 'all 0.2s', background: active ? 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 100%)' : '#f5f3ff', color: active ? 'white' : '#7c3aed', textTransform: 'capitalize' }),
    assessmentsGrid: { display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(350px, 1fr))', gap: '1.5rem' },
    assessmentCard: (status) => ({ background: status === 'locked' ? '#faf5ff' : 'white', borderRadius: '20px', padding: '1.5rem', boxShadow: '0 4px 20px rgba(139, 92, 246, 0.08)', border: '1px solid #ede9fe', transition: 'all 0.3s', opacity: status === 'locked' ? 0.7 : 1 }),
    cardHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' },
    cardIcon: (status) => ({ width: '50px', height: '50px', borderRadius: '14px', display: 'flex', alignItems: 'center', justifyContent: 'center', background: status === 'completed' ? 'linear-gradient(135deg, #10b981 0%, #059669 100%)' : status === 'in-progress' ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : status === 'locked' ? '#ede9fe' : 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 100%)', color: status === 'locked' ? '#a78bfa' : 'white' }),
    statusBadge: (status) => ({ padding: '0.375rem 0.875rem', borderRadius: '20px', fontSize: '0.75rem', fontWeight: 600, background: status === 'completed' ? '#d1fae5' : status === 'in-progress' ? '#fef3c7' : status === 'locked' ? '#f5f3ff' : '#ede9fe', color: status === 'completed' ? '#059669' : status === 'in-progress' ? '#d97706' : status === 'locked' ? '#a78bfa' : '#7c3aed' }),
    recommendedBadge: { padding: '0.25rem 0.625rem', borderRadius: '12px', fontSize: '0.7rem', fontWeight: 600, background: 'linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%)', color: '#78350f', marginLeft: '0.5rem' },
    cardTitle: { fontSize: '1.15rem', fontWeight: 700, color: '#1f2937', marginBottom: '0.75rem' },
    metaRow: { display: 'flex', gap: '1rem', marginBottom: '1rem', flexWrap: 'wrap' },
    metaItem: { display: 'flex', alignItems: 'center', gap: '0.25rem', fontSize: '0.8rem', color: '#6b7280' },
    difficultyBadge: (diff) => ({ padding: '0.25rem 0.625rem', borderRadius: '12px', fontSize: '0.7rem', fontWeight: 600, background: diff === 'Advanced' ? '#fee2e2' : diff === 'Intermediate' ? '#fef3c7' : '#d1fae5', color: diff === 'Advanced' ? '#dc2626' : diff === 'Intermediate' ? '#d97706' : '#059669' }),
    progressSection: { marginBottom: '1rem' },
    progressLabel: { display: 'flex', justifyContent: 'space-between', marginBottom: '0.5rem', fontSize: '0.8rem' },
    progressBar: { height: '8px', background: '#ede9fe', borderRadius: '4px', overflow: 'hidden' },
    progressFill: (progress) => ({ width: `${progress}%`, height: '100%', background: 'linear-gradient(90deg, #f59e0b 0%, #d97706 100%)', borderRadius: '4px' }),
    scoreSection: { display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: '#f0fdf4', borderRadius: '12px', marginBottom: '1rem' },
    scoreCircle: { width: '50px', height: '50px', borderRadius: '50%', background: 'linear-gradient(135deg, #10b981 0%, #059669 100%)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '1.1rem' },
    badgeInfo: { display: 'flex', alignItems: 'center', gap: '0.5rem', color: '#059669', fontWeight: 600, fontSize: '0.875rem' },
    lockMessage: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.75rem', background: '#f5f3ff', borderRadius: '10px', fontSize: '0.8rem', color: '#7c3aed', marginBottom: '1rem' },
    actionButton: (status) => ({ width: '100%', padding: '0.875rem', borderRadius: '12px', fontWeight: 600, cursor: status === 'locked' ? 'not-allowed' : 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: '0.5rem', background: status === 'completed' ? 'white' : status === 'in-progress' ? 'linear-gradient(135deg, #f59e0b 0%, #d97706 100%)' : status === 'locked' ? '#ede9fe' : 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 100%)', color: status === 'completed' ? '#7c3aed' : status === 'locked' ? '#a78bfa' : 'white', border: status === 'completed' ? '2px solid #7c3aed' : 'none', boxShadow: status !== 'locked' && status !== 'completed' ? '0 4px 15px rgba(139, 92, 246, 0.3)' : 'none' }),
