...
const Jobs = () => {
  ...
  const styles = {
    ...
    title: { fontSize: '1.75rem', fontWeight: 700, background: 'linear-gradient(135deg, #7c3aed, #8b5cf6)', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '0.5rem' },
    subtitle: { color: '#6b7280', fontSize: '1rem' },
    statsRow: { display: 'flex', alignItems: 'center', gap: '0.5rem', marginTop: '1rem', color: '#7c3aed', fontWeight: 500 },
    searchSection: { display: 'flex', gap: '1rem', marginBottom: '1.5rem', flexWrap: 'wrap' },
    ...
    searchInput: { width: '100%', padding: '1rem 1rem 1rem 3rem', borderRadius: '12px', border: '2px solid #ede9fe', fontSize: '1rem', outline: 'none', boxSizing: 'border-box', background: 'white' },
    filterButton: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '1rem 1.5rem', borderRadius: '12px', border: '2px solid #ede9fe', background: 'white', cursor: 'pointer', fontWeight: 500, color: '#7c3aed' },
    filtersRow: { display: 'flex', gap: '0.75rem', marginBottom: '2rem', flexWrap: 'wrap' },
    filterChip: (active) => ({ padding: '0.5rem 1.25rem', borderRadius: '25px', border: 'none', cursor: 'pointer', fontWeight: 500, fontSize: '0.875rem', transition: 'all 0.2s', background: active ? 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 100%)' : '#f5f3ff', color: active ? 'white' : '#7c3aed' }),
    jobsGrid: { display: 'grid', gap: '1.25rem', gridTemplateColumns: 'repeat(auto-fit, minmax(320px, 1fr))' },
    jobCard: { background: 'white', borderRadius: '18px', padding: '1.25rem', boxShadow: '0 4px 20px rgba(139, 92, 246, 0.08)', border: '1px solid #ede9fe', transition: 'all 0.3s' },
    jobHeader: { display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '1rem' },
    jobTitleRow: { display: 'flex', alignItems: 'flex-start', gap: '1rem' },
    companyLogo: (color) => ({ width: '48px', height: '48px', borderRadius: '12px', background: `linear-gradient(135deg, ${color || '#7c3aed'} 0%, ${color || '#8b5cf6'}cc 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '1.1rem' }),
    jobTitle: { fontSize: '1.15rem', fontWeight: 700, color: '#1f2937', marginBottom: '0.25rem' },
    companyInfo: { display: 'flex', alignItems: 'center', gap: '1rem', color: '#6b7280', fontSize: '0.9rem' },
    companyItem: { display: 'flex', alignItems: 'center', gap: '0.25rem' },
    matchBadge: (match) => ({ padding: '0.5rem 1rem', borderRadius: '25px', fontWeight: 700, fontSize: '0.9rem', background: match >= 90 ? 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 100%)' : match >= 80 ? 'linear-gradient(135deg, #a78bfa 0%, #c4b5fd 100%)' : match >= 70 ? 'linear-gradient(135deg, #c4b5fd 0%, #ddd6fe 100%)' : '#f5f3ff', color: match >= 70 ? 'white' : '#7c3aed' }),
    featuredBadge: { padding: '0.375rem 0.875rem', borderRadius: '20px', background: 'linear-gradient(135deg, #fbbf24 0%, #f59e0b 100%)', color: '#78350f', fontWeight: 600, fontSize: '0.75rem', display: 'flex', alignItems: 'center', gap: '0.25rem' },
    badgesRow: { display: 'flex', gap: '0.5rem', alignItems: 'center' },
    bookmarkBtn: { padding: '0.5rem', borderRadius: '10px', border: 'none', background: '#f5f3ff', cursor: 'pointer', color: '#8b5cf6' },
    description: { color: '#6b7280', marginBottom: '1rem', lineHeight: 1.55, fontSize: '0.9rem' },
    skillsRow: { display: 'flex', gap: '0.5rem', flexWrap: 'wrap', marginBottom: '1.25rem' },
    skillTag: { padding: '0.375rem 0.875rem', background: '#f5f3ff', color: '#7c3aed', borderRadius: '20px', fontSize: '0.8rem', fontWeight: 500 },
    metaRow: { display: 'flex', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.25rem' },
    metaItem: { display: 'flex', alignItems: 'center', gap: '0.375rem', color: '#6b7280', fontSize: '0.875rem' },
    cardFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '1rem', borderTop: '1px solid #f5f3ff' },
    matchText: { fontWeight: 600, fontSize: '0.9rem' },
    applyButton: { display: 'flex', alignItems: 'center', gap: '0.5rem', padding: '0.65rem 1.25rem', borderRadius: '10px', border: 'none', background: 'linear-gradient(135deg, #7c3aed 0%, #8b5cf6 100%)', color: 'white', fontWeight: 600, cursor: 'pointer', textDecoration: 'none', fontSize: '0.85rem', transition: 'all 0.2s', boxShadow: '0 4px 15px rgba(139, 92, 246, 0.3)' },
    postedText: { color: '#9ca3af', fontSize: '0.8rem' }
  };
    ...
    <div style={styles.container}>
      ...
      <div style={styles.jobsGrid}>
        {filteredJobs.map((job, index) => (
          <motion.div key={job.id} style={styles.jobCard} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} whileHover={{ boxShadow: '0 10px 40px rgba(0,0,0,0.12)', transform: 'translateY(-4px)' }}>
              ...
              <div style={styles.jobTitleRow}>
                ...
                <div>
                  ...
                  <div style={styles.companyInfo}>
                    <span style={styles.companyItem}><Building2 size={16} /> {job.company}</span>
                    <span style={styles.companyItem}><MapPin size={16} /> {job.location}</span>
                  </div>
                </div>
              </div>
              <div style={styles.badgesRow}>
                {job.featured && <span style={styles.featuredBadge}><Star size={12} /> Featured</span>}
                <span style={styles.matchBadge(job.match)}>{job.match}% Match</span>
                <button style={styles.bookmarkBtn}><Bookmark size={18} /></button>
              </div>
            </div>
            ...
            <div style={styles.metaRow}>
              <span style={styles.metaItem}><Briefcase size={16} /> {job.type}</span>
              <span style={styles.metaItem}><Clock size={16} /> {job.experience}</span>
              <span style={styles.metaItem}><DollarSign size={16} /> ₹{job.salary}</span>
              <span style={styles.metaItem}><Users size={16} /> {job.applicants} applicants</span>
            </div>

            <div style={styles.cardFooter}>
              <div>
                <div style={{ ...styles.matchText, color: job.match >= 85 ? '#10b981' : job.match >= 70 ? '#7c3aed' : '#f59e0b' }}>{job.match}% Match</div>
                <span style={styles.postedText}>Posted {job.posted}</span>
              </div>
