...
const Jobs = () => {
  ...
  const styles = {
    ...
    jobsGrid: { display: 'grid', gap: '0.85rem', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))' },
    jobCard: { background: 'white', borderRadius: '14px', padding: '0.85rem', boxShadow: '0 2px 12px rgba(139, 92, 246, 0.06)', border: '1px solid #ede9fe', transition: 'all 0.3s', display: 'flex', flexDirection: 'column', minHeight: '280px' },
    ...
    companyLogo: (color) => ({ width: '36px', height: '36px', borderRadius: '10px', background: `linear-gradient(135deg, ${color || '#2E073F'} 0%, ${color || '#2E073F'}cc 100%)`, display: 'flex', alignItems: 'center', justifyContent: 'center', color: 'white', fontWeight: 700, fontSize: '0.9rem', flexShrink: 0 }),
    jobTitle: { fontSize: '0.9rem', fontWeight: 700, color: '#1f2937', marginBottom: '0.15rem', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' },
    ...
    bookmarkBtn: { padding: '0.35rem', borderRadius: '8px', border: 'none', background: '#f5f3ff', cursor: 'pointer', color: '#2E073F' },
    description: { color: '#6b7280', marginBottom: '0.65rem', lineHeight: 1.5, fontSize: '0.75rem', flex: 1, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 3, WebkitBoxOrient: 'vertical' },
    skillsRow: { display: 'flex', gap: '0.35rem', flexWrap: 'wrap', marginBottom: '0.75rem', maxHeight: '52px', overflow: 'hidden' },
    ...
    metaItem: { display: 'flex', alignItems: 'center', gap: '0.25rem', color: '#6b7280', fontSize: '0.7rem', whiteSpace: 'nowrap' },
    cardFooter: { display: 'flex', justifyContent: 'space-between', alignItems: 'center', paddingTop: '0.65rem', borderTop: '1px solid #f5f3ff', marginTop: 'auto' },
    matchText: { fontWeight: 600, fontSize: '0.75rem' },
    applyButton: { display: 'flex', alignItems: 'center', gap: '0.35rem', padding: '0.45rem 0.85rem', borderRadius: '8px', border: 'none', background: 'linear-gradient(135deg, #2E073F 0%, #2E073F 100%)', color: 'white', fontWeight: 600, cursor: 'pointer', textDecoration: 'none', fontSize: '0.72rem', transition: 'all 0.2s', boxShadow: '0 3px 10px rgba(139, 92, 246, 0.25)', whiteSpace: 'nowrap' },
        ...
        {filteredJobs.map((job, index) => (
          <motion.div key={job.id} style={styles.jobCard} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.1 }} whileHover={{ boxShadow: '0 8px 30px rgba(0,0,0,0.1)', transform: 'translateY(-3px)' }}>
            ...
            <div style={styles.cardFooter}>
              <div>
                <div style={{ ...styles.matchText, color: job.match >= 85 ? '#10b981' : job.match >= 70 ? '#2E073F' : '#f59e0b' }}>{job.match}% Match</div>
